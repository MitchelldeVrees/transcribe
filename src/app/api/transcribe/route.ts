...
Belangrijkste punten: performance regressies, nieuwe onboarding flow, en een pilot bij DMA Europe.`;

const MOCK_SUMMARY = `
<div class="prose max-w-none">
  <h2 class="text-xl font-semibold">Samenvatting</h2>
  <p>Het team heeft de Q4-planning afgestemd. Focus ligt op performance fixes, nieuwe onboarding, en pilot-uitrol.</p>
  <ul class="list-disc pl-5">
    <li>Performance: regressies in audio-transcript pipeline prioriteit P1</li>
    <li>Onboarding: A/B-test met vereenvoudigde stappen</li>
    <li>Pilot: DMA Europe krijgt feature flags + support</li>
  </ul>
</div>
`.trim();
...
<ul class="list-disc pl-5">
  <li>[Jan] Profiling rapport opleveren (vrijdag 16:00)</li>
  <li>[Sara] Onboarding copy finaliseren & handoff aan dev (ma)</li>
  <li>[Mitch] Feature flags klaarzetten voor pilot (di)</li>
</ul>
`.trim();

const MOCK_QNA = `
<h3 class="text-lg font-semibold mb-2">Vragen & Antwoorden</h3>
<ol class="list-decimal pl-5">
  <li><strong>Wanneer start de pilot?</strong> Volgende week dinsdag.</li>
  <li><strong>Welke KPI’s meten we?</strong> Doorlooptijd onboarding, task success, NPS.</li>
  <li><strong>Wie monitort regressies?</strong> Jan voert monitoring uit en rapporteert in Slack.</li>
</ol>
`.trim();
...
export async function POST(request: NextRequest) {
  ...
  const MOCK =
    process.env.MOCK_TRANSCRIBE === 'true' || search.get('mock') === '1';
  ...
  try {
    ...
    if (MOCK) {
      return NextResponse.json(
        { text: MOCK_TEXT, summary: MOCK_SUMMARY, actionItems: MOCK_ACTION_ITEMS, qna: MOCK_QNA },
        { status: 200 }
      );
    }
