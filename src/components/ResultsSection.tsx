...
export default function ResultsSection({
  ...
  exportToWord,           // shows "Exporteer naar Word"
  true
)}
...
function renderPanel(
  ...
  rawText: string,
  onCopy: () => void,
  extraAction?: () => void,            // if provided, shows "Exporteer naar Word"
  preserveWhitespace = false
) {
  let lines: any[];
  if (Array.isArray(content)) lines = content;
  else if (preserveWhitespace) lines = (content as string).split("\n");
  else lines = (content as string).split("\n\n");
  ...
  return (
    <div className="rounded-lg border border-gray-200 bg-white">
      ...
      {/* Body */}
      <div
        className={`max-h-72 overflow-y-auto p-4 text-gray-800 ${preserveWhitespace ? "whitespace-pre-wrap" : ""}`}
      >
        {lines.map((line, idx) => (
